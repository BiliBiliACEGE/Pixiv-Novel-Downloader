...
class PixivNovelDownloader(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Pixiv小说下载器")
        ...
        self.novel_id_input.setPlaceholderText("输入小说ID、小说链接或系列链接")
        ...
        self.batch_input.setPlaceholderText("输入多个小说ID、小说链接或系列链接，每行一个")
        # 关键设置：禁用自动格式化，只接受纯文本
        ...
        batch_layout.addWidget(QLabel("批量下载列表:"))
        batch_layout.addWidget(self.batch_input)
        batch_layout.addSpacing(10)
        batch_layout.addWidget(batch_download_btn)
        batch_layout.addStretch()
        
        self.top_tab_widget.addTab(batch_download_tab, "批量下载")
        ...
        # 进度标签
        self.progress_label = QLabel("当前状态: 空闲")
    ...
    def clear_download_history(self):
        ...
        reply = QMessageBox.question(self, "确认", "确定要清空下载记录吗？此操作不可恢复。",
    ...
    def extract_content_id(self, input_text):
        ...
        # 如果没有匹配任何模式，抛出详细错误
        error_msg = f"无法从输入中提取有效的内容ID: '{input_text}'"
    ...
    def download_novel(self, novel_id=None):
        ...
        try:
            ...
            if novel_id is False:
                ...
                QMessageBox.critical(self, "错误", "检测到无效的小说ID参数")
                return
            ...
            if novel_id is None:
                ...
                if not input_text:
                    ...
                    QMessageBox.warning(self, "警告", "请输入小说ID、小说链接或系列链接")
                    return
                ...
                if not content_id.isdigit():
                    error_msg = f"无效的内容ID格式: '{content_id}'"
                ...
                if novel_id_str == "False":
                    ...
                    QMessageBox.critical(self, "错误", "无效的小说ID格式: 'False'")
                    return
                
                # 验证传入的novel_id是否为有效数字
                if not novel_id_str.isdigit():
                    error_msg = f"无效的小说ID格式: '{novel_id_str}'"
                    logging.error(error_msg)
                    raise ValueError(error_msg)
    ...
    def download_single_novel(self, novel_id):
        ...
        try:
            ...
            if not isinstance(novel_id, str) or not novel_id.isdigit():
                error_msg = f"无效的小说ID格式: '{novel_id}'"
                logging.error(error_msg)
                raise ValueError(error_msg)
            ...
            # 更新进度状态
            self.progress.setValue(0)
            self.progress_label.setText("当前状态: 下载中")
            self.progress_info.setText(f"正在获取小说信息: ID {novel_id}")
            ...
            if novel_data.get("error"):
                error_msg = novel_data["error"].get("message", "未知错误")
            ...
            self.progress_info.setText(f"正在保存小说: 《{novel_title}》")
            QApplication.processEvents()
            ...
            self.progress_label.setText("当前状态: 完成")
            self.progress_info.setText(f"小说《{novel_title}》下载完成")
            ...
            # 显示成功消息并返回主页
            QMessageBox.information(self, "成功", f"小说《{novel_title}》下载完成！")
            self.switch_tab(0)
            ...
            QMessageBox.critical(self, "错误", error_msg)
            self.progress.setValue(0)
            self.progress_label.setText("当前状态: 错误")
            self.progress_info.setText(f"下载失败: {str(e)}")
    ...
    def download_series(self, series_id):
        ...
        try:
            ...
            if not isinstance(series_id, str) or not series_id.isdigit():
                error_msg = f"无效的系列ID格式: '{series_id}'"
                logging.error(error_msg)
                raise ValueError(error_msg)
            ...
            # 更新进度状态
            self.progress.setValue(0)
            self.progress_label.setText("当前状态: 获取系列信息")
            self.progress_info.setText(f"正在获取系列信息: ID {series_id}")
            QApplication.processEvents()
            ...
            if response.status_code == 404:
                error_msg = f"系列ID {series_id} 不存在"
                logging.error(error_msg)
                raise Exception(error_msg)
            ...
            if series_data.get("error"):
                error_msg = series_data["error"].get("message", "未知错误")
                logging.error(f"API返回错误: {error_msg}")
                QMessageBox.critical(self, "错误", error_msg)
                return
            ...
            # 批量下载系列中的小说
            total = len(novel_ids)
            self.progress_label.setText(f"当前状态: 下载系列《{series_title}》")
            self.progress.setValue(0)
            ...
            for i, novel_id in enumerate(novel_ids):
                ...
                self.progress_info.setText(f"正在下载 {i+1}/{total} (ID: {novel_id})")
            ...
            self.progress.setValue(100)
            self.progress_label.setText("当前状态: 系列下载完成")
            self.progress_info.setText(f"系列《{series_title}》下载完成! 成功: {success_count}/{total}")
            ...
            # 显示成功消息并返回主页
            QMessageBox.information(self, "完成", f"系列《{series_title}》下载完成! 成功下载 {success_count}/{total} 本小说")
            self.switch_tab(0)
            logging.info(f"系列下载完成! 成功: {success_count}/{total}")
            
        except Exception as e:
            error_msg = f"系列下载失败: {str(e)}"
            logging.error(error_msg, exc_info=True)
            QMessageBox.critical(self, "错误", error_msg)
            self.progress.setValue(0)
            self.progress_label.setText("当前状态: 错误")
            self.progress_info.setText(f"系列下载失败: {str(e)}")
        ...
        try:
            ...
            while offset < total:
                ...
                if content_data.get("error"):
                    error_msg = content_data["error"].get("message", "未知错误")
    ...
    def batch_download(self):
        ...
        try:
            ...
            if not input_text:
                ...
                QMessageBox.warning(self, "警告", "请输入多个小说ID、小说链接或系列链接")
                return
            ...
            if errors:
                ...
                QMessageBox.warning(self, "输入错误", f"以下行包含无效输入:\n\n{error_msg}")
                    
            if not content_ids:
                logging.warning("批量下载中没有找到有效的内容ID")
                QMessageBox.warning(self, "警告", "没有找到有效的内容ID")
                return
            ...
            self.progress_label.setText("当前状态: 批量下载中")
            self.progress.setValue(0)
            ...
            for i, (content_type, content_id) in enumerate(content_ids):
                progress = int((i / total) * 100)
                self.progress.setValue(progress)
                self.progress_info.setText(f"正在下载 {i+1}/{total} (ID: {content_id})")
                QApplication.processEvents()  # 更新UI
            ...
            self.progress.setValue(100)
            self.progress_label.setText("当前状态: 批量下载完成")
            self.progress_info.setText(f"批量下载完成! 成功: {success_count}/{total}")
            
            # 显示成功消息并返回主页
            QMessageBox.information(self, "完成", f"批量下载完成! 成功下载 {success_count}/{total} 个项目")
            self.switch_tab(0)
            ...
            self.progress.setValue(0)
            self.progress_label.setText("当前状态: 错误")
            self.progress_info.setText(f"批量下载失败: {str(e)}")

class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("设置")
        self.setFixedSize(600, 550)  # 增加高度以容纳更多内容
        ...
        self.format_combo.addItems(["TXT (纯文本)", "HTML (网页格式)", "Markdown (MD格式)"])
        ...
        open_folder_label = QLabel("下载完成后操作:")
        open_folder_label.setStyleSheet("font-weight: 500;")
        
        self.open_folder_checkbox = QCheckBox("下载完成后打开文件夹")
